...
export default function ReviewActions({ screeningId, currentStatus }: Props) {
  ...
  async function handleAction(action: string) {
    if (action === "FLAGGED" && !note.trim()) {
      setError("Please add a reason before flagging so the screener knows what to correct.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    <div className="border rounded p-3 bg-light">
      <div className="mb-2">
        <label className="form-label small fw-semibold mb-1">
          Review Note
          <span className="text-danger ms-1">* required when flagging</span>
        </label>
        <textarea className="form-control form-control-sm" rows={2}
          value={note} onChange={e => setNote(e.target.value)}
          placeholder="e.g. Wrong date of birth, please correct..." />
      </div>
